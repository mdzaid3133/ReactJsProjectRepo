        ...
        <section className="text-gray-600 body-font">
            <div className="container px-5 py-8 md:py-16 mx-auto">
                <div class="lg:w-1/2 w-full mb-6 lg:mb-10">
                    <h1 class="sm:text-3xl text-2xl font-medium title-font mb-2 text-gray-900" style={{ color: mode === 'dark' ? 'white' : '' }}>Our {category} Collection</h1>
                    <div class="h-1 w-20 bg-pink-600 rounded"></div>
                </div>

                <div className="flex flex-wrap -m-4">

                    {product.filter(item => item.category.toLowerCase() === category.toLocaleLowerCase()).map((item, index) => {

                             //const {title,price,description,imageUrl} = item
                            return (
                                <div onClick={() => window.location.href = `/productinfo/${item.id}`}
                                    className="p-4 md:w-1/4  drop-shadow-lg relative" key={index}>
                                    <div className="h-full border-2 hover:shadow-gray-100 hover:shadow-2xl transition-shadow duration-300 ease-in-out    border-gray-200 border-opacity-60 rounded-2xl overflow-hidden" style={{ backgroundColor: mode === 'dark' ? 'rgb(46 49 55)' : '', color: mode === 'dark' ? 'white' : '', }} >
                                        ...
                                        <div className="p-5 border-t-2 ">
                                            ...
                                            <h1 className="leading title-font text-lg font-medium text-gray-900 mb-3" style={{ color: mode === 'dark' ? 'white' : '', }}>{item.title}</h1>
